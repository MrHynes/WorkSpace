...
public class AdminInfoServlet extends HttpServlet {
	...
	public void updateInfo(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		for (Element element : allChildren) {
			if (userName.equals(element.getChild("name").getText())) {
				element.getChild("relname").setText(relName);
				element.getChild("sex").setText(sex);
				element.getChild("birthday").setText(birthday);
				element.getChild("email").setText(email);
				element.getChild("telphone").setText(telephone);
				element.getChild("address").setText(address);
				element.getChild("postcode").setText(postCode);
			}
		}
		saveXML(doc, path);
		req.getRequestDispatcher("/backend/admInfo?task=getAdmin&flag=showmyinfo").forward(req, resp);
	...
	public void modifyPwd(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String pwd = null;
		for (Element element : allChildren) {
			if (element.getChild("name").getText().equals(userName)) {
